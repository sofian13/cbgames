      ...
      <div className="flex flex-1 flex-col items-center justify-center p-4" style={{ background: "#060606" }}>
        <div className="w-full max-w-md space-y-5">
          ...
          <div className="rounded-xl border border-cyan-500/20 bg-cyan-500/5 p-4 text-center">
            ...
            <p className="text-sm font-serif text-cyan-200/80">{king?.name} est le Roi</p>
            <p className="text-xs text-cyan-200/40 font-mono">{king?.score} pts</p>
          </div>

          {isKing && (
            <p className="text-xs text-cyan-400/60 text-center font-sans">Tu es le Roi ! +10 pts/tour. Défends-toi !</p>
          )}
          ...
          <div className="flex gap-2 flex-wrap justify-center">
            {state.players.map(p => (
              <span key={p.id} className={cn("text-xs font-sans px-2 py-1 rounded",
                p.isKing ? "text-cyan-400/80 bg-cyan-500/10" :
          ...
          <div className="space-y-2">
            {[...state.players].sort((a, b) => b.score - a.score).map((p, i) => (
              <div key={p.id} className={cn("flex items-center justify-between rounded-lg border p-3",
                p.isKing ? "border-cyan-500/20 bg-cyan-500/5" : "border-white/[0.06] bg-white/[0.03]")}>
