...
/* ─────────────────────────────────────────────────────────────
   UNDERCOVER — Refonte v3
   Mirror 1:1 des maquettes (noir/dossier + blobs partout).
   ───────────────────────────────────────────────────────────── */
...
const ROLE_COLOR: Record<Role, string> = {
  civil: "#3DDC97", undercover: "#FF3EA5", mrwhite: "#FFD23F",
};
const ROLE_LABEL: Record<Role, string> = {
  civil: "Civil", undercover: "Undercover", mrwhite: "Mr. White",
};
...
// ═════════════════════════════════════════════════════════════
//  Composant principal — route par phase
// ═════════════════════════════════════════════════════════════
...
function Btn({
  children, tone = "primary", disabled = false, onClick, full = true,
  icon, sub, style = {},
}: {
  ...
  return (
    ...
    >
      ...
      <span style={{ display: "flex", flexDirection: "column", alignItems: sub ? "flex-start" : "center", textAlign: "left" }}>
        <span>{children}</span>
...
// SpyMascot — Mascot + lunettes de soleil dorées
function SpyMascot({
  id, size = 80, tilt = -3, mood = "sus", cheering = false, crown = false, arms = false, delay = 0,
}: {
  ...
  return (
    <div style={{ position: "relative", display: "inline-block", width: size, height: size * 1.15 + (crown ? size * 0.3 : 0) }}>
      ...
      <div style={{
        ...
        top: (crown ? size * 0.3 : 0) + size * 0.32,
        width: w, height: h,
...
function Confetti({ count = 40, accent = "#FFD23F" }: { count?: number; accent?: string }) {
  ...
  return (
    <>
      ...
      <style>{`@keyframes uc-confetti { 0% { transform: translateY(-30px) rotate(0); opacity: 1; } 100% { transform: translateY(110vh) rotate(720deg); opacity: 1; } }`}</style>
    </>
  );
}

// ═════════════════════════════════════════════════════════════
//  PHASE — WAITING (briefing + composition + start)
//  Mirror maquette 04 + 05 condensé.
// ═════════════════════════════════════════════════════════════
  ...
  return (
    <>
      ...
      <NavBar
        sub="DOSSIER · CASE #14"
        title="Briefing"
        right={<Tag color="#FF3EA5">BLUFF · {total}/7</Tag>}
      />
      ...
      <div style={{ position: "relative", height: 200, display: "flex", alignItems: "flex-end", justifyContent: "center", marginBottom: 8 }}>
        ...
        {featured[1] && (
          <div style={{ position: "absolute", left: "10%", bottom: 0 }}>
            <SpyMascot id={featured[1].id} size={68} tilt={-10} mood="shifty" arms delay={0.3} />
          </div>
        )}
        {featured[2] && (
          <div style={{ position: "absolute", right: "10%", bottom: 0 }}>
            <SpyMascot id={featured[2].id} size={68} tilt={8} mood="wink" arms delay={0.6} />
          </div>
        )}
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
          {state.players.map((p) => (
            <div key={p.id} style={{
              ...
              {p.id === state.players[0]?.id && p.id !== myId && (
...
function RevealPhase({ state, sendAction, myId }: { state: UCState; sendAction: (a: Record<string, unknown>) => void; myId: string }) {
  ...
  return (
    <>
      ...
      {/* Mini-blob au-dessus de la carte */}
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ fontFamily: "var(--font-mono, 'JetBrains Mono'), monospace", fontSize: 10, fontWeight: 800, letterSpacing: 2 }}>
            CONFIDENTIEL
          </div>
          <div style={{ fontFamily: "var(--font-mono, 'JetBrains Mono'), monospace", fontSize: 10, fontWeight: 800, letterSpacing: 1, color: "#7A3008" }}>
            MANCHE {state.round}
          </div>
        </div>

        <div style={{ fontFamily: "var(--font-mono, 'JetBrains Mono'), monospace", fontSize: 11, color: "#7A3008", textTransform: "uppercase", letterSpacing: 1.5, fontWeight: 700 }}>
      ...
      <div style={{
        ...
        <div style={{ fontSize: 12, color: "white", fontWeight: 600, lineHeight: 1.3 }}>
          Personne d'autre ne doit voir l'écran.
        </div>
      </div>

      <div style={{ marginTop: "auto" }}>
        <Btn tone="gold" onClick={() => sendAction({ action: "ack-word" })}>
          J'ai mémorisé · passer
        </Btn>
      </div>
    </>
  );
...
// ═════════════════════════════════════════════════════════════
//  PHASE — CLUE (KEY — maquette 08)
//  Tour de parole vertical + Revoir mot (modal) + Voter
// ═════════════════════════════════════════════════════════════
function CluePhase({ state, sendAction, myId }: { state: UCState; sendAction: (a: Record<string, unknown>) => void; myId: string }) {
  ...
  // Construit la file dans l'ordre serveur, en gardant les morts à la fin
  ...
  return (
    <>
      <NavBar
        sub={`Manche ${state.round} · ordre de parole`}
        title="À qui le tour ?"
        right={<Tag color="#3DDC97">{alive} EN VIE</Tag>}
      />
          ...
          return (
            <div key={p.id} style={{
              ...
              background: isCurrent
                ? "linear-gradient(90deg, rgba(255,210,63,0.20) 0%, rgba(255,210,63,0.04) 100%)"
                : "transparent",
      ...
      {/* Input d'indice — seulement pour le joueur courant */}
      {isMyTurn && !dead && (
        <div style={{
          display: "flex", gap: 8, padding: 6, borderRadius: 14,
          background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,210,63,0.30)",
          marginBottom: 12,
        }}>
          <input
            autoFocus
            ...
            style={{
              ...
              fontSize: 16, padding: "10px", outline: "none", fontWeight: 600,
              fontFamily: "inherit",
            }}
          />
          <Btn tone="gold" full={false} onClick={submitClue} style={{ padding: "10px 16px" }}>
            Envoyer →
          </Btn>
        </div>
      )}
      ...
      <div style={{ marginTop: "auto", display: "flex", flexDirection: "column", gap: 10 }}>
        ...
        {/* Le bouton Voter n'est utile que pour l'hôte (forcer la phase) si tout le monde a parlé */}
      ...
      {/* Modal "Revoir mon mot" */}
      {showWord && (
        <WordModal
          role={state.myRole ?? "civil"}
          word={state.myWord}
...
function WordModal({ role, word, onClose }: { role: Role; word: string | null; onClose: () => void }) {
  ...
  const label = ROLE_LABEL[role];
  useEffect(() => {
    const t = setTimeout(onClose, 6000); // auto-hide après 6s
  ...
  return (
    <div onClick={onClose} style={{
      ...
      <div onClick={(e) => e.stopPropagation()} style={{
        ...
        <div style={{ fontFamily: "var(--font-mono, monospace)", fontSize: 10, fontWeight: 800, letterSpacing: 2 }}>
          CONFIDENTIEL · auto-fermeture 6s
        </div>
    ...
    <>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 14 }}>
        {alive.map((p) => {
          ...
          return (
            <button
              key={p.id}
              disabled={isMe || dead || me?.hasVoted}
              onClick={() => !isMe && setPicked(p.id)}
              style={{
                position: "relative", padding: "16px 12px 14px", borderRadius: 18,
                background: isPicked
                  ? "linear-gradient(160deg, rgba(255,62,165,0.35), rgba(0,0,0,0.4))"
            ...
            >
              {isMe && (
                <span style={{ position: "absolute", top: 8, left: 8, padding: "2px 7px", borderRadius: 99, background: "#FFD23F", color: "#1A0E2E", fontSize: 8, fontWeight: 900, letterSpacing: 0.5 }}>TOI</span>
              )}
              {tally > 0 && (
                <span style={{ position: "absolute", top: 8, right: 8, padding: "2px 8px", borderRadius: 99, background: "#FF3EA5", color: "white", fontSize: 9, fontWeight: 800 }}>
                  {tally} vote{tally > 1 ? "s" : ""}
                </span>
              )}
              <div style={{ display: "flex", justifyContent: "center", marginTop: 4 }}>
                <Mascot color={colorOf(p.id)} size={64} mood={isPicked ? "shocked" : isMe ? "neutral" : "shifty"} arms bob={false} delay={Math.random()} />
              </div>
              <div style={{
                fontFamily: "var(--font-display, 'Bricolage Grotesque'), system-ui",
                fontSize: 18, color: "white", marginTop: 10,
                lineHeight: 1, fontWeight: 800, letterSpacing: -0.3,
              }}>
                {p.name}
              </div>
              {isPicked ? (
                <div style={{
                  ...
                  borderRadius: 10, background: "#FF3EA5",
                  color: "white", fontSize: 11, fontWeight: 800, letterSpacing: 0.4,
                }}>✕ Ton vote</div>
              ) : isMe ? (
                <div style={{
                  marginTop: 8, padding: "6px 0", borderRadius: 10,
      ...
      <div style={{ marginBottom: 16 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 6 }}>
          ...
          <span style={{ fontFamily: "var(--font-mono, monospace)", fontSize: 12, fontWeight: 800 }}>
            {voted} / {alive.length}
          </span>
        </div>
      ...
      <div style={{ marginTop: "auto" }}>
        <Btn
          tone="danger"
          disabled={!picked || dead || (me?.hasVoted ?? false)}
          onClick={cast}
...
// ═════════════════════════════════════════════════════════════
//  PHASE — VOTE RESULT / ROUND END (maquette 10)
// ═════════════════════════════════════════════════════════════
  ...
  return (
    <>
      ...
      <div style={{
        ...
        width: 800, height: 800, maxWidth: "180vw", maxHeight: "180vw",
      ...
      <style>{`@keyframes uc-rotate { to { transform: translate(-50%, -50%) rotate(360deg); } }`}</style>
      ...
      {elim ? (
        <>
          ...
          <div style={{ position: "relative", display: "flex", flexDirection: "column", alignItems: "center", padding: "12px 0 20px" }}>
            ...
            <div style={{
              fontFamily: "var(--font-display, 'Bricolage Grotesque'), system-ui",
              fontSize: 44, marginTop: 14, letterSpacing: -1.5, fontWeight: 900,
            }}>{elim.name}</div>
          </div>
          ...
          {(elim.word !== null || state.civilWord) && (
            <div style={{
              ...
              <div>
                ...
                <div style={{
                  fontFamily: "var(--font-display, 'Bricolage Grotesque'), system-ui",
                  fontSize: 26, color, fontWeight: 900, marginTop: 4,
                }}>{elim.word ?? "Aucun mot"}</div>
              </div>
              ...
              <div style={{ textAlign: "right" }}>
                ...
                <div style={{
                  fontFamily: "var(--font-display, 'Bricolage Grotesque'), system-ui",
                  fontSize: 22, color: "white", fontWeight: 900, marginTop: 4,
                }}>{role ? ROLE_LABEL[role] : "—"}</div>
              </div>
            </div>
          )}
        </>
      ...
      {isMe && (
        <div style={{
          ...
          <input
            ...
            style={{
              flex: 1, background: "transparent", border: "none", color: "white",
              fontSize: 18, padding: "10px", outline: "none",
  ...
  return (
    <>
      ...
      <div style={{ textAlign: "center", marginBottom: 8 }}>
        <h1 style={{
          fontFamily: "var(--font-display, 'Bricolage Grotesque'), system-ui",
          fontSize: 48, margin: 0, color: "white",
          letterSpacing: -2, lineHeight: 0.9, fontWeight: 900,
      ...
      <div style={{ position: "relative", height: 200, display: "flex", alignItems: "flex-end", justifyContent: "center", marginBottom: 14 }}>
        <div style={{
          position: "absolute", left: "50%", top: 20, transform: "translateX(-50%)",
          width: 360, height: 220, maxWidth: "100%",
          background: `radial-gradient(ellipse 50% 50% at 50% 50%, ${accent}40, transparent 70%)`,
          pointerEvents: "none",
        }} />
        {sorted[1] && (
          <div style={{ position: "absolute", left: "12%", bottom: 0 }}>
            <SpyMascot id={sorted[1].id} size={72} tilt={-8} mood="happy" arms delay={0.2} />
          </div>
        )}
        {sorted[2] && (
          <div style={{ position: "absolute", right: "12%", bottom: 0 }}>
            <SpyMascot id={sorted[2].id} size={68} tilt={6} mood="wink" arms delay={0.3} />
          </div>
        )}
        {sorted[0] && (
          <div style={{ position: "relative", zIndex: 2 }}>
            <SpyMascot id={sorted[0].id} size={130} tilt={-2} mood="happy" cheering arms crown delay={0} />
          </div>
        )}
      </div>
      ...
      {state.civilWord && state.undercoverWord && (
        <div style={{
          ...
          <div>
            <Mono>Civils</Mono>
            <div style={{ fontFamily: "var(--font-display, 'Bricolage Grotesque'), system-ui", fontSize: 20, color: "#3DDC97", fontWeight: 900 }}>
              {state.civilWord}
            </div>
          </div>
          ...
          <div style={{ textAlign: "right" }}>
            ...
            <div style={{ fontFamily: "var(--font-display, 'Bricolage Grotesque'), system-ui", fontSize: 20, color: "#FF3EA5", fontWeight: 900 }}>
        ...
        {sorted.map((p, i) => {
          ...
          return (
            <div key={p.id} style={{
              ...
              <span style={{ width: 16, fontFamily: "var(--font-mono, monospace)", fontWeight: 800, fontSize: 11, color: i === 0 ? "#FFD23F" : "rgba(255,255,255,0.5)" }}>
                #{i + 1}
              </span>
              <Mascot color={colorOf(p.id)} size={28} mood={win ? "wink" : "dead"} bob={false} shadow={false} />
              <span style={{ flex: 1, fontWeight: 700, fontSize: 13, color: "white" }}>{p.name}</span>
              <span style={{
              ...
              <span style={{
                ...
                color: win ? "#3DDC97" : "rgba(255,255,255,0.5)", width: 38, textAlign: "right",
              }}>{p.score}</span>
            </div>
          );
        })}
...
function Stat({ n, label, color, sub }: { n: number; label: string; color: string; sub?: string }) {
  return (
    <div style={{
      ...
      <div style={{
        fontFamily: "var(--font-display, 'Bricolage Grotesque'), system-ui",
        fontSize: 26, color, lineHeight: 1, fontWeight: 900,
      }}>{n}</div>
...
function Stepper({
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
      ...
      <span style={{ width: 22, textAlign: "center", fontWeight: 800, fontFamily: "var(--font-display, system-ui)", fontSize: 18 }}>{value}</span>
